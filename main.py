...
class MeuGame:
    def __init__(self):
        ...
        self.jump_sound = pg.mixer.Sound("data/pulo.wav")
        ...
        self.jump_l = [pg.transform.scale(pg.image.load(f"data/jump_left.png"), (64, 64))]
        self.jump_r = [pg.transform.scale(pg.image.load(f"data/jump_right.png"), (64, 64))]
        ...
        self.background = pg.transform.scale(pg.image.load("data/background.png"), (840, 540))
    ...
    def move(self, keys):
        ...
        if keys[pg.K_a]:
            ...
            if self.player_collider(): self.player_pos[0] += 5
                ...
                if on_ground: self.player_animation = 3
                moving = True
            ...
            if self.player_collider(): self.player_pos[0] -= 5
                ...
                self.player_animation = 0 
                if on_ground: self.player_animation = 2
                moving = True
    ...
    def player(self):
        ...
        # 1. Determina a lista base
        if self.player_animation == 0: img_list = self.idle_r
        elif self.player_animation == 1: img_list = self.idle_l
        elif self.player_animation == 2: img_list = self.walk_r
        elif self.player_animation == 3: img_list = self.walk_l

        
        if self.player_vertical_speed != 0:
            if self.player_animation in [0, 2]:
                image_to_draw = self.jump_r[0]
            else:
                image_to_draw = self.jump_l[0]
        else:
           
            if self.player_animation_frame >= len(img_list):
                self.player_animation_frame = 0
            image_to_draw = img_list[int(self.player_animation_frame)]

        self.window.blit(image_to_draw, self.player_pos)

    def draw_map(self):
        for y in range(len(self.map)):
            for x in range(len(self.map[0])):
                if self.map[y][x] != "":
                    self.window.blit(self.ground, (x * 64 - self.scroll, y * 64))
# ---------------- LOOP ----------------
jogo = MeuGame()
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN:
            if event.key == pg.K_SPACE:
                if jogo.player_vertical_speed == 0:
                    jogo.player_vertical_speed = jogo.player_jump_force
                    jogo.jump_sound.play() 
                    jogo.player_pos[1] -= 2
